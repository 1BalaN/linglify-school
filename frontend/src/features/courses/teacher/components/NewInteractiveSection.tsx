...
export const NewInteractiveSection = ({
  ...
  const { t } = useTranslation('platform', { keyPrefix: 'lessonBuilder.interactive' })
  ...
  return (
    <div className="space-y-4 rounded-xl border border-purple-200 bg-purple-50/50 p-4 dark:border-purple-900/30 dark:bg-purple-950/20">
      ...
      <VideoUpload value={videoUrl} onChange={onVideoChange} label={tv('interactive')} />
      ...
      <div className="rounded-lg border border-purple-200/60 bg-purple-100/30 px-3 py-2 text-xs text-purple-700 dark:border-purple-800/30 dark:bg-purple-900/20 dark:text-purple-300">
        {t('howTo')}{' '}
        <code className="rounded bg-purple-200/50 px-1 dark:bg-purple-800/40">___</code> {t('howToMid')}{' '}
        <em>{t('example')}</em>
      </div>

      <div>
        <h4 className="mb-3 text-sm font-semibold">{t('exercises', { count: exercises.length })}</h4>
        ...
        <Button variant="outline" size="sm" onClick={onAddExercise} type="button" className="mt-3 w-full">
