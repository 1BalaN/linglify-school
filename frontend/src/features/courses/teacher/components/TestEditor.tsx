...
interface TestEditorProps {
  isFinalTest: boolean
  onIsFinalTestChange: (v: boolean) => void
  passThreshold: string
  onPassThresholdChange: (v: string) => void
  testTimeLimit: string
  onTestTimeLimitChange: (v: string) => void
  testShuffleQuestions: boolean
  onShuffleQuestionsChange: (v: boolean) => void
  testShuffleOptions: boolean
  onShuffleOptionsChange: (v: boolean) => void
  questions: EditQuestionForm[]
  onAddQuestion: () => void
  onRemoveQuestion: (id: string) => void
  onPatchQuestion: (id: string, patch: Partial<EditQuestionForm>) => void
  onAddOption: (qId: string) => void
  onRemoveOption: (qId: string, oId: string) => void
  onPatchOption: (qId: string, oId: string, patch: Partial<{ text: string; isCorrect: boolean }>) => void
}
    ...
    <div className="space-y-4 rounded-xl border border-amber-200 bg-amber-50/50 p-4 dark:border-amber-900/30 dark:bg-amber-950/20">
      <div className="flex items-center justify-between gap-2 text-sm font-medium text-amber-700 dark:text-amber-400">
        ...
        <label className="flex cursor-pointer items-center gap-2 text-xs font-medium text-amber-800 dark:text-amber-300">
          <input
            type="checkbox"
            checked={isFinalTest}
            onChange={e => onIsFinalTestChange(e.target.checked)}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <Input
            type="number"
            value={testTimeLimit}
            onChange={e => onTestTimeLimitChange(e.target.value)}
            min={1}
      ...
      <div className="max-h-[520px] space-y-4 overflow-y-auto pr-1 scroll-soft">
        <div className="mb-3 flex items-center justify-between">
          <h4 className="text-sm font-semibold">{t('questions', { count: questions.length })}</h4>
        </div>
            ...
            <div key={q.id} className="rounded-lg border border-border bg-card p-4">
              <div className="mb-3 flex items-start gap-2">
                ...
                <div className="flex-1">
                  <textarea
                    value={q.text}
                    onChange={e => onPatchQuestion(q.id, { text: e.target.value })}
              ...
              <div className="mb-2">
                ...
                <textarea
                  value={q.explanation}
                  onChange={e => onPatchQuestion(q.id, { explanation: e.target.value })}
              ...
              <div className="mb-2 flex items-center gap-2">
                <label className="flex cursor-pointer items-center gap-1.5 text-xs text-muted-foreground">
                  <input
                    ...
                    onChange={e => onPatchQuestion(q.id, { isMultiple: e.target.checked })}
                ...
                {q.options.map(opt => (
                  <div key={opt.id} className="flex items-center gap-2">
                    <input
                      ...
                      onChange={e => {
                        if (!q.isMultiple) {
                          q.options.forEach(o => onPatchOption(q.id, o.id, { isCorrect: false }))
                        }
                        onPatchOption(q.id, opt.id, { isCorrect: e.target.checked })
                      }}
                    ...
                    <Input
                      value={opt.text}
                      onChange={e => onPatchOption(q.id, opt.id, { text: e.target.value })}
