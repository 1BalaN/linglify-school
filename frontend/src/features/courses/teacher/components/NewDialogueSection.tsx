...
export const NewDialogueSection = ({
  ...
  const { t } = useTranslation('platform', { keyPrefix: 'lessonBuilder.dialogue' })
  ...
  return (
    <div className="space-y-4 rounded-xl border border-sky-200 bg-sky-50/50 p-4 dark:border-sky-900/30 dark:bg-sky-950/20">
      ...
      <VideoUpload value={videoUrl} onChange={onVideoChange} label={tv('dialogue')} />
      ...
      <div>
        <h4 className="mb-3 text-sm font-semibold">{t('steps', { count: steps.length })}</h4>
        ...
        <Button className="mt-3 w-full" variant="outline" size="sm" type="button" onClick={onAddStep}>
          <Plus className="mr-1 h-3 w-3" />
          {t('addStep')}
        </Button>
      </div>
