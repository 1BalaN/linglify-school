...
│   │   │   ├── faq/
│   │   │   └── contact/
│   │   ├── cron/                # node-cron задачи
│   │   └── shared/              # AppError, middlewares, socket, redis
...
- **Подписка преподавателя**: месячная (30 BYN) / годовая (240 BYN) через Stripe Checkout
- **30 дней бесплатного пробного периода** при регистрации как учитель
- Без активной подписки опубликовать курс невозможно (guard на статус PENDING_REVIEW)
- Синхронизация статуса подписки с Stripe API (`POST /subscriptions/sync`) — независимо от вебхука
- **Комиссия платформы 25%** с каждой продажи курса; 75% — преподавателю
- Учёт выручки в `CourseRevenue` (каждая транзакция разбита: сумма, platformFee, teacherEarning)
- Преподаватель видит: всего заработано, выплачено, ожидает, доступно к выводу (`/teacher/earnings`)
- Запрос на выплату с указанием реквизитов (карта / телефон / IBAN), минимум 10 BYN
- Прогресс выплаты: PENDING → PROCESSING → COMPLETED (визуальный progress-bar)
- Администратор видит реквизиты и управляет выплатами (`/admin/revenue`)
- Управление подпиской прямо в профиле (вкладка «Подписка» для учителей)
- Визуальный бейдж срока подписки в шапке профиля
