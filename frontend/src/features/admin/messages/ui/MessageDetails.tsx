import { MailOpen, CheckCircle, Trash2, X, Save } from 'lucide-react'
  ...
  return (
    <div className="rounded-2xl glass-card p-6 backdrop-blur-xl">
      <div className="flex justify-between mb-6 pb-4 border-b border-border">
        ...
        {message.isReplied ? (
          <Badge icon={<CheckCircle />} text={t('badgeAnswered')} />
        ) : message.isRead ? (
          <Badge icon={<MailOpen />} text={t('badgeRead')} />
        ) : null}
      </div>
      ...
      <div className="flex gap-2">
        ...
        <Button variant="outline" className="ml-auto text-destructive" onClick={onDelete}>
